...
The team name is The Watson Four; the project name remains PortFlow AI. The source deck in F:/Project is untouched. Its placeholder names/links and unsupported claims are corrected in this repository's copy. PDF export is preferred by the guide, but slides.pptx is accepted.

Artifact tooling: src/tools/update_existing_deck.py requires python-pptx (available in the Codex bundled document runtime), not an application dependency. Run with --source pointing to the original supplied PPT. The original raster artwork remains beneath native editable text overlays.
